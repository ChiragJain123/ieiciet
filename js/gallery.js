...
let fetchImagesAndShow = () => {};
{
	...
	fetchImagesAndShow = async () => {
		...
		let snapshot = await db.ref('gallery').get();
		let galleryData = snapshot.val();
		document.getElementsByClassName('overlay-loader')[0].style.opacity = 0;

		setTimeout(() => {
			document
				.getElementsByClassName('overlay-loader')[0]
				.classList.add('d-none');
			document.getElementsByClassName('overlay-loader')[0].style.opacity =
				null;
		}, 200);
